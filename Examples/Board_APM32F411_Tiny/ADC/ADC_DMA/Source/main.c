...
int main(void)
{
    ...
    while (1)
    {
        if (DMA_ReadStatusFlag(DMA2_Stream2, DMA_FLAG_TCIFLG2))
        {
            ...
            Delay(0xFFFFFF);
            DMA_ClearStatusFlag(DMA2_Stream2, DMA_FLAG_TCIFLG2);
        }
    }
}
...
void ADC_Init(void)
{
    ...
    ADC_CommonConfig(ADC2, &adcCommonConfig);
    
    ADC_Config(ADC2, &adcConfig);
    ADC_ConfigRegularChannel(ADC2, ADC_CHANNEL_0, 1, ADC_SAMPLETIME_112CYCLES);
