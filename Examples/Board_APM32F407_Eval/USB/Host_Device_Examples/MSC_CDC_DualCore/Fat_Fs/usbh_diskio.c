...
DSTATUS USBH_GetDiskStatus(BYTE lun)
{
    ...
    if (USBH_MSC_DevStatus(&gUsbHostHS, USB_LUN_NUMBER))
    {
        stat &= ~STA_NOINIT;
    }
...
DRESULT USBH_DiskRead(BYTE lun, BYTE *buff, LBA_t sector, UINT count)
{
    ...
    if (USBH_MSC_DevRead(&gUsbHostHS, USB_LUN_NUMBER, sector, buff, count) == USBH_OK)
    {
        res = RES_OK;
    }
    else
    {
        USBH_MSC_ReadDevInfo(&gUsbHostHS, USB_LUN_NUMBER, &storageInfo);
...
DRESULT USBH_DiskWrite(BYTE lun, const BYTE *buff, LBA_t sector, UINT count)
{
    ...
    if (USBH_MSC_DevWrite(&gUsbHostHS, USB_LUN_NUMBER, sector, (BYTE*)buff, count) == USBH_OK)
    {
        res = RES_OK;
    }
    else
...
DRESULT USBH_DiskIoctl(BYTE lun, BYTE cmd, void *buff)
{
    ...
    switch (cmd)
    {
        ...
        case GET_SECTOR_SIZE:
            if (USBH_MSC_ReadDevInfo(&gUsbHostHS, USB_LUN_NUMBER, &storageInfo) == USBH_OK)
            {
