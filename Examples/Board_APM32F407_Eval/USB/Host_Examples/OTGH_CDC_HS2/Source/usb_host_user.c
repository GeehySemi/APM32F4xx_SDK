    ...
    switch (hostInfo.state)
    {
        ...
        case USBH_APP_READY:
            if (!BOARD_PBGetState(BUTTON_KEY1))
            {
                ...
                if (!BOARD_PBGetState(BUTTON_KEY1))
                {
                    if (USBH_CDC_GetTxDataState(&gUsbHostHS) == USBH_CDC_DATA_TX_IDLE)
                    {
                        USBH_USR_LOG("USB host send data");
                        (void)memset(cdcTxBuffer, 0xAA, USBH_CDC_DATA_SIZE);
                        USBH_CDC_SendData(&gUsbHostHS, cdcTxBuffer, USBH_CDC_DATA_SIZE);
                    }
                }
            ...
            if (!BOARD_PBGetState(BUTTON_KEY2))
            {
                ...
                if (!BOARD_PBGetState(BUTTON_KEY2))
                {
                    if (USBH_CDC_GetRxDataState(&gUsbHostHS) == USBH_CDC_DATA_RX_IDLE)
                    {
                        USBH_USR_LOG("USB host receive data");
                        (void)memset(cdcRxBuffer, 0, USBH_CDC_DATA_SIZE);
                        USBH_CDC_ReceiveData(&gUsbHostHS, cdcRxBuffer, USBH_CDC_DATA_SIZE);
                    }
                }
