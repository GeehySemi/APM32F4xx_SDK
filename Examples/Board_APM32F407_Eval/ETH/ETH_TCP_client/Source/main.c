...
int main(void)
{
    ...
    BOARD_LCDPrintString(30, 40, "This is a ETH Demo!", LCD_COLOR_USER);

    printf("This is a ETH TCP Client Demo!\n");
    BOARD_LCDPrintString(30, 40, "This is a ETH Client Demo!", LCD_COLOR_USER);
    ...
    BOARD_LCDPrintString(30, 80, LCDDisplayBuf, LCD_COLOR_USER);
    ...
    IP_ADDR3);
    printf("%s",LCDDisplayBuf);
    BOARD_LCDPrintString(30, 120, LCDDisplayBuf, LCD_COLOR_USER);
    ...
    NETMASK_ADDR3);
    printf("%s",LCDDisplayBuf);
    BOARD_LCDPrintString(30, 160, LCDDisplayBuf, LCD_COLOR_USER);
    ...
    GW_ADDR3);
    printf("%s",LCDDisplayBuf);
    BOARD_LCDPrintString(30, 200, LCDDisplayBuf, LCD_COLOR_USER);
    ...
    COMP_PORT);
    printf("%s",LCDDisplayBuf);
    BOARD_LCDPrintString(30, 240, LCDDisplayBuf, LCD_COLOR_USER);

    printf("\n\rKEY1: Connect TCP server\n");
    printf("KEY2: Disconnect TCP server\n");

    BOARD_LCDPrintString(30, 280, "KEY1: Connect TCP server\n", LCD_COLOR_USER);
    BOARD_LCDPrintString(30, 320, "KEY2: Disconnect TCP server\n", LCD_COLOR_USER);

    while(1)
    {

        if ((BOARD_PBGetState(BUTTON_KEY1)==0)&&(flag==0))
        {
            BOARD_LEDOn(LED1);
            if (EthLinkStatus == 0)
            {
        ...
        if ((BOARD_PBGetState(BUTTON_KEY2)==0)&&(flag==1))
        {
            BOARD_LEDOff(LED1);
