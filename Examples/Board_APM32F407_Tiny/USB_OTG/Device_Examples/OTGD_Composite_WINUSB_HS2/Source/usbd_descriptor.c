...
{
    USBD_DESC_INFO_T descInfo;
    USBD_EP_DESC_T *winusb1EpInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB1_IN_EP_ADDR);
    USBD_EP_DESC_T *winusb1EpOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB1_OUT_EP_ADDR);

    USBD_EP_DESC_T *winusb2EpInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB2_IN_EP_ADDR);
    USBD_EP_DESC_T *winusb2EpOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB2_OUT_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (winusb1EpInDesc != NULL)
        {
            winusb1EpInDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }

        if (winusb1EpOutDesc != NULL)
        {
            winusb1EpOutDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }

        if (winusb2EpInDesc != NULL)
        {
            winusb2EpInDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }

        if (winusb2EpOutDesc != NULL)
        {
            winusb2EpOutDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }
    }
    else
    {
        if (winusb1EpInDesc != NULL)
        {
            winusb1EpInDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }

        if (winusb1EpOutDesc != NULL)
        {
            winusb1EpOutDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }

        if (winusb2EpInDesc != NULL)
        {
            winusb2EpInDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }

        if (winusb2EpOutDesc != NULL)
        {
            winusb2EpOutDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }
    }
