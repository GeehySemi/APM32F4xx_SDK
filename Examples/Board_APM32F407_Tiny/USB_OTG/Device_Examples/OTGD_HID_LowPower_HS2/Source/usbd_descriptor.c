...
{
    USBD_DESC_INFO_T descInfo;
    USBD_EP_DESC_T *epInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_HID_IN_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (epInDesc != NULL)
        {
            epInDesc->wMaxPacketSize = USBD_HID_IN_EP_SIZE;
            epInDesc->bInterval = USBD_HID_FS_INTERVAL;
        }
    }
    else
    {
        ...
        {
            epInDesc->wMaxPacketSize = USBD_HID_IN_EP_SIZE;
            epInDesc->bInterval = USBD_HID_HS_INTERVAL;
        }
    }
