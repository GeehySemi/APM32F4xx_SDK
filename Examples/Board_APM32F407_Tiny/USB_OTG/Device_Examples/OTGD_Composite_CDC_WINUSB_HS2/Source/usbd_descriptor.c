...
uint8_t USBD_ConfigDesc[USBD_CONFIG_DESCRIPTOR_SIZE] =
{
    ...
    WINUSB_HS_MAX_PACKET_SIZE & 0xFF,
    WINUSB_HS_MAX_PACKET_SIZE >> 8,
...
{
    USBD_DESC_INFO_T descInfo;
    USBD_EP_DESC_T *winusbEpInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB_IN_EP_ADDR);
    USBD_EP_DESC_T *winusbEpOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_WINUSB_OUT_EP_ADDR);

    USBD_EP_DESC_T *cdcEpCmdDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_CMD_EP_ADDR);
    USBD_EP_DESC_T *cdcEpOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_DATA_OUT_EP_ADDR);
    USBD_EP_DESC_T *cdcEpInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_DATA_IN_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (winusbEpInDesc != NULL)
        {
            winusbEpInDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }

        if (winusbEpOutDesc != NULL)
        {
            winusbEpOutDesc->wMaxPacketSize = WINUSB_FS_MAX_PACKET_SIZE;
        }

        if (cdcEpCmdDesc != NULL)
        {
            cdcEpCmdDesc->bInterval = USBD_CDC_FS_INTERVAL;
        }

        if (cdcEpOutDesc != NULL)
        {
            cdcEpOutDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
        
        if (cdcEpInDesc != NULL)
        {
            cdcEpInDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
    }
    else
    {
        if (winusbEpInDesc != NULL)
        {
            winusbEpInDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }

        if (winusbEpOutDesc != NULL)
        {
            winusbEpOutDesc->wMaxPacketSize = WINUSB_HS_MAX_PACKET_SIZE;
        }

        if (cdcEpCmdDesc != NULL)
        {
            cdcEpCmdDesc->bInterval = USBD_CDC_HS_INTERVAL;
        }

        if (cdcEpOutDesc != NULL)
        {
            cdcEpOutDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
        
        if (cdcEpInDesc != NULL)
        {
            cdcEpInDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
    }
