...
static USBD_DESC_INFO_T USBD_HS_ConfigDescHandler(uint8_t usbSpeed)
{
    ...
    USBD_EP_DESC_T *cdc1CmdDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC1_CMD_EP_ADDR);
    USBD_EP_DESC_T *cdc1OutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC1_DATA_OUT_EP_ADDR);
    USBD_EP_DESC_T *cdc1InDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC1_DATA_IN_EP_ADDR);

    USBD_EP_DESC_T *cdc2OutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC2_DATA_OUT_EP_ADDR); 
    USBD_EP_DESC_T *cdc2InDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC2_DATA_IN_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (cdc1CmdDesc != NULL)
        {
            cdc1CmdDesc->bInterval = USBD_CDC_FS_INTERVAL;
        }

        if (cdc1OutDesc != NULL)
        {
            cdc1OutDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
        
        if (cdc1InDesc != NULL)
        {
            cdc1InDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }

        if (cdc2OutDesc != NULL)
        {
            cdc2OutDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }

        if (cdc2InDesc != NULL)
        {
            cdc2InDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
    }
    else
    {
        if (cdc1CmdDesc != NULL)
        {
            cdc1CmdDesc->bInterval = USBD_CDC_HS_INTERVAL;
        }

        if (cdc1OutDesc != NULL)
        {
            cdc1OutDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
        
        if (cdc1InDesc != NULL)
        {
            cdc1InDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }

        if (cdc2OutDesc != NULL)
        {
            cdc2OutDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }

        if (cdc2InDesc != NULL)
        {
            cdc2InDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
    }
