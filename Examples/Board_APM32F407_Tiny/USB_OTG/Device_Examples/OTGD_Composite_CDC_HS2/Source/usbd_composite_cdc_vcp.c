...
static USBD_STA_T USBD_HS_CDC_ItfInit(void);
static USBD_STA_T USBD_HS_CDC_ItfDeInit(void);
static USBD_STA_T USBD_HS_CDC_ItfCtrl(uint8_t command, uint8_t *buffer, uint16_t length, uint8_t cdcnum);
static USBD_STA_T USBD_HS_CDC_ItfSendEnd(uint8_t epNum, uint8_t *buffer, uint32_t *length);
static USBD_STA_T USBD_HS_CDC_ItfReceive(uint8_t *buffer, uint32_t *length, uint8_t cdcnum);
...
static USBD_STA_T USBD_HS_CDC_ItfInit(void)
{
    ...
    {
        USBD_Composite_CDC_ConfigRxBuffer(&gUsbDeviceHS, cdcRxBuffer[i], i);
        USBD_Composite_CDC_ConfigTxBuffer(&gUsbDeviceHS, cdcTxBuffer, 0, i);
    }
 ...
 * @retval      USB device operation status
 */
USBD_STA_T USBD_HS_CDC_ItfSend(uint8_t *buffer, uint16_t length, uint8_t cdcnum)
{
    USBD_STA_T usbStatus = USBD_OK;
    ...
    USBD_Composite_CDC_ConfigTxBuffer(&gUsbDeviceHS, buffer, length, cdcnum);
    
    usbStatus = USBD_Composite_CDC_TxPacket(&gUsbDeviceHS, cdcnum);
...
static USBD_STA_T USBD_HS_CDC_ItfReceive(uint8_t *buffer, uint32_t *length, uint8_t cdcnum)
{
    ...
    USBD_Composite_CDC_ConfigRxBuffer(&gUsbDeviceHS, &buffer[0], cdcnum);
    
    USBD_Composite_CDC_RxPacket(&gUsbDeviceHS, cdcnum);
