...
void USB_DevUserApplication(void)
{
    ...
    {
        case USER_APP_INIT:
            interval = USBD_Composite_CUSTOM_HID_ReadInterval(&gUsbDeviceHS);
        ...
        case USER_APP_RUN:
            ...
            if (!BOARD_PBGetState(BUTTON_KEY2))
            {
                ...
                if (!BOARD_PBGetState(BUTTON_KEY2))
                {
                    if(i > 29)
                    {
                        ...
                        USBD_Composite_CUSTOM_HID_TxReport(&gUsbDeviceHS, (uint8_t*)report, 8);
                        BOARD_DelayMs(20);
                        report[2] = 0;
                        USBD_Composite_CUSTOM_HID_TxReport(&gUsbDeviceHS, (uint8_t*)report, 8);
                        BOARD_DelayMs(20);
            ...
            if(gWinusbItf.state == USBD_WINUSB_ITF_REV_UPDATE)
            {
                ...
                USBD_HS_WINUSB_ItfSend((uint8_t *)winusbRxBuffer, (uint16_t)gWinusbItf.rxUpdateLen);
