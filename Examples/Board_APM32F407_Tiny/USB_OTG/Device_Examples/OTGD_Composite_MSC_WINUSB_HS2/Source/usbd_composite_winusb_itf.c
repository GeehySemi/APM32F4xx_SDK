...
static USBD_STA_T USBD_HS_WINUSB_ItfInit(void);
static USBD_STA_T USBD_HS_WINUSB_ItfDeInit(void);
static USBD_STA_T USBD_HS_WINUSB_ItfCtrl(uint8_t command, uint8_t *buffer, uint16_t length);
static USBD_STA_T USBD_HS_WINUSB_ItfSendEnd(uint8_t epNum, uint8_t *buffer, uint32_t *length);
static USBD_STA_T USBD_HS_WINUSB_ItfReceive(uint8_t *buffer, uint32_t *length);
...
static USBD_STA_T USBD_HS_WINUSB_ItfInit(void)
{
    ...
    USBD_Composite_WINUSB_ConfigRxBuffer(&gUsbDeviceHS, winusbRxBuffer);
    USBD_Composite_WINUSB_ConfigTxBuffer(&gUsbDeviceHS, winusbTxBuffer, 0);
...
USBD_STA_T USBD_HS_WINUSB_ItfSend(uint8_t *buffer, uint16_t length)
{
    ...
    USBD_Composite_WINUSB_ConfigTxBuffer(&gUsbDeviceHS, buffer, length);
    
    usbStatus = USBD_Composite_WINUSB_TxPacket(&gUsbDeviceHS);
...
static USBD_STA_T USBD_HS_WINUSB_ItfReceive(uint8_t *buffer, uint32_t *length)
{
    ...
    USBD_Composite_WINUSB_ConfigRxBuffer(&gUsbDeviceHS, &buffer[0]);
    
    USBD_Composite_WINUSB_RxPacket(&gUsbDeviceHS);
