...
{
    USBD_DESC_INFO_T descInfo;
    USBD_EP_DESC_T *epOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_MSC_OUT_EP_ADDR);
    USBD_EP_DESC_T *epInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_MSC_IN_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (epInDesc != NULL)
        {
            epInDesc->wMaxPacketSize = USBD_MSC_FS_MP_SIZE;
        }

        if (epOutDesc != NULL)
        {
            epOutDesc->wMaxPacketSize = USBD_MSC_FS_MP_SIZE;
        }
    }
    else
    {
        if (epInDesc != NULL)
        {
            epInDesc->wMaxPacketSize = USBD_MSC_HS_MP_SIZE;
        }

        if (epOutDesc != NULL)
        {
            epOutDesc->wMaxPacketSize = USBD_MSC_HS_MP_SIZE;
        }
    }
