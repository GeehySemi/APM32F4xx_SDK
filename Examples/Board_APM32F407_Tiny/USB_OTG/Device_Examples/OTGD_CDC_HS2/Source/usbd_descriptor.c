...
{
    USBD_DESC_INFO_T descInfo;
    USBD_EP_DESC_T *epCmdDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_CMD_EP_ADDR);
    USBD_EP_DESC_T *epOutDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_DATA_OUT_EP_ADDR);
    USBD_EP_DESC_T *epInDesc = USBD_GetEpDesc(USBD_ConfigDesc, USBD_CDC_DATA_IN_EP_ADDR);

    if (usbSpeed == USBD_SPEED_FS)
    {
        if (epCmdDesc != NULL)
        {
            epCmdDesc->bInterval = USBD_CDC_FS_INTERVAL;
        }

        if (epOutDesc != NULL)
        {
            epOutDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
        
        if (epInDesc != NULL)
        {
            epInDesc->wMaxPacketSize = USBD_CDC_FS_MP_SIZE;
        }
    }
    else
    {
        if (epCmdDesc != NULL)
        {
            epCmdDesc->bInterval = USBD_CDC_HS_INTERVAL;
        }

        if (epOutDesc != NULL)
        {
            epOutDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
        
        if (epInDesc != NULL)
        {
            epInDesc->wMaxPacketSize = USBD_CDC_HS_MP_SIZE;
        }
    }
