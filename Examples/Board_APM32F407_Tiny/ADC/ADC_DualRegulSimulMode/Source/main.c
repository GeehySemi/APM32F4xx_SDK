...
int main(void)
{
    ...
    while (1)
    {
        if (DMA_ReadStatusFlag(DMA2_Stream0, DMA_FLAG_TCIFLG0))
        {
            ...
            ADC2_CH1_ConvertedValue = (float)DMA_DualConvertedValue[1] / 4095 * 3.3f;

            ADC1_CH1_ConvertedValue = (float)DMA_DualConvertedValue[2] / 4095 * 3.3f;
            ADC2_CH2_ConvertedValue = (float)DMA_DualConvertedValue[3] / 4095 * 3.3f;

            LOGI(tag, "ADC CDATA = 0x%04X,0x%04X,0x%04X,0x%04X \n",
            DMA_DualConvertedValue[0],
            DMA_DualConvertedValue[1],
            DMA_DualConvertedValue[2],
            DMA_DualConvertedValue[3]);

            LOGI(tag, "ADC CH0: ADC1 Voltage    = %f V \r\n", ADC1_CH0_ConvertedValue);
            LOGI(tag, "ADC CH1: ADC1 Voltage    = %f V \r\n", ADC1_CH1_ConvertedValue);
            LOGI(tag, "ADC CH1: ADC2 Voltage    = %f V \r\n", ADC2_CH1_ConvertedValue);
            LOGI(tag, "ADC CH2: ADC2 Voltage    = %f V \r\n", ADC2_CH2_ConvertedValue);
...
void ADC_Init(void)
{
    ...
    adcCommonConfig.mode        = ADC_MODE_DUAL_REGSIMULT;
    ...
    ADC_ConfigRegularChannel(ADC2, ADC_CHANNEL_1, 1, ADC_SAMPLETIME_3CYCLES);
    ADC_ConfigRegularChannel(ADC2, ADC_CHANNEL_2, 2, ADC_SAMPLETIME_3CYCLES);
