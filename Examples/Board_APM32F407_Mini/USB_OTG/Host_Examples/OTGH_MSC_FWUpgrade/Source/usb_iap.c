...
void USB_IAP_Upload(void)
{
    ...
    if (status == RESET)
    {
        ...
        {
            /* Read flash memory */
            while ((indexOffset < REMAIN_FLASH_SIZE) && (gUsbHostFS.devInfo.connectedStatus == ENABLE))
            {
                for (readIndex = 0; readIndex < IAP_BUFFER_SIZE; readIndex++)
                {
                    /* Check the flash data */
                    if (indexOffset + readIndex < REMAIN_FLASH_SIZE)
                    {
                        temp = readIndex;
...
IAP_OP_T USB_IAP_ProgramFlashMemory(void)
{
    ...
    while ((readFlag == 1) && (gUsbHostFS.devInfo.connectedStatus == ENABLE))
    {
        ...
        readSizeTemp = ((bytesRead + 0x3)/0x4)*0x4;
