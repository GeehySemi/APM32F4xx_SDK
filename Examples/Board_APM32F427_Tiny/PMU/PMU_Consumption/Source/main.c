...
void BOR_Config(FMC_OPT_BOR_T level)
{
    ...
    if((BOROptionByteTemp & 0x0C) != level)
    {
        ...
        LOG_Print("The BOR value is set from 0x%02X to 0x%02X\r\n",BOROptionByteTemp,level);
    }
    else
    {
        LOG_Print("The BOR setting is the same as in option bytes\r\n");
    }
}
