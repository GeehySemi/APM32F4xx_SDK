...
int main(void)
{
    ...
    LOGI(tag, "When input Voltage of PA0 voltage is lower than 0.62v or higher than 2.27V,LED2 is on, otherwise, LED2 is off\r\n");
...
void ADC_Init(void)
{
    ...
    ADC_EnableInterrupt(ADC1, ADC_INT_AWD | ADC_INT_EOC);
 ...
 * @retval    None
 */
void ADC_IRQRequest(void)
{
    uint16_t adcData = 0;
    uint16_t voltage = 0;

    if (ADC_ReadStatusFlag(ADC1, ADC_FLAG_AWD))
    {
        /* Turn on LED2 */
        BOARD_LEDOn(LED2);
        iAWD_Flag = 1;
        /* Clear ADC1 AWD pending interrupt bit */
        ADC_ClearStatusFlag(ADC1, ADC_FLAG_AWD);
    }
