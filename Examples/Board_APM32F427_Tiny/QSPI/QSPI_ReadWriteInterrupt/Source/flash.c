...
#define TRANSMISSIONMODE 3

#ifndef TRANSMISSIONMODE
#define TRANSMISSIONMODE 1
#endif
/* Private typedef ********************************************************/
...
void Flash_Read(uint32_t addr, uint8_t *rBuf, uint16_t rLen)
{
    ...
    QSPI_Quad_ReadDMA(&param);
#else
    QSPI_Quad_ReadIT(&param);
#endif
}
...
#elif (TRANSMISSIONMODE == 2)
    QSPI_Quad_WriteDMA(&param);
#else
    QSPI_Quad_WriteIT(&param);
#endif
