...
void USBD_HardwareInit(USBD_INFO_T* usbInfo)
{
    ...
    if (usbInfo->devSpeed == USBD_SPEED_FS2)
    {
        ...
        /* Configure USB OTG */
        RCM_EnableAHB1PeriphClock(RCM_AHB1_PERIPH_OTG_FS2);

        /* Configure USB OTG GPIO */
        RCM_EnableAHB1PeriphClock(RCM_AHB1_PERIPH_GPIOB);

        /* Link structure */
        usbDeviceHandler.usbGlobal    = USB_OTG_FS2;
        usbDeviceHandler.usbDevice    = USB_OTG_FS2_D;
        usbDeviceHandler.usbFifo      = USB_OTG_FS2_FIFO;
        usbDeviceHandler.usbPower     = USB_OTG_FS2_PWR;
        ...
        GPIO_ConfigPinAF(GPIOB, GPIO_PIN_SOURCE_14, GPIO_AF_OTG_FS2);
        GPIO_ConfigPinAF(GPIOB, GPIO_PIN_SOURCE_15, GPIO_AF_OTG_FS2);
        ...
        NVIC_EnableIRQRequest(OTG_FS2_IRQn, 1, 0);

        /* Low power */
        if (usbDeviceHandler.usbCfg.lowPowerStatus == ENABLE)
        {
            USBD_OTG_FS2_WAKEUP_EINT_CLEAR_FLAG();
            USBD_OTG_FS2_WAKEUP_EINT_ENABLE_RISING_EDGE();
            USBD_OTG_FS2_WAKEUP_EINT_ENABLE_IT();

            NVIC_EnableIRQRequest(OTG_FS2_WKUP_IRQn, 1, 0);
        }
        ...
        /* Configure FIFO */
        /* Configure RX FIFO */
        USB_OTG_ConfigRxFifoSize(usbDeviceHandler.usbGlobal, USBD_FS2_RX_FIFO_SIZE);

        /* Configure TX FIFO */
        USBD_OTG_ConfigDeviceTxFifo(&usbDeviceHandler, 0, USBD_FS2_TX_FIFO_0_SIZE);
        USBD_OTG_ConfigDeviceTxFifo(&usbDeviceHandler, 1, USBD_FS2_TX_FIFO_1_SIZE);
...
void USBD_HardwareReset(USBD_INFO_T* usbInfo)
{
    if (usbInfo->devSpeed == USBD_SPEED_FS2)
    {
        RCM_DisableAHB1PeriphClock(RCM_AHB1_PERIPH_OTG_FS2);
        
        NVIC_DisableIRQRequest(OTG_FS2_IRQn);
    }
}
...
USBD_STA_T USBD_SetTestModeCallback(USBD_INFO_T* usbInfo, uint8_t testMode)
{
    ...
    USBD_SetTestMode(usbInfo->dataPoint, testMode);
