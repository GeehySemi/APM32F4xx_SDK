...
void USBH_HardwareInit(USBH_INFO_T* usbInfo)
{
    ...
    if (usbInfo->hostSpeed == USBH_SPEED_FS2)
    {
        ...
        /* Link structure */
        usbHostHandler.usbGlobal    = USB_OTG_FS2;
        usbHostHandler.usbHost      = USB_OTG_FS2_H;
        usbHostHandler.usbFifo      = USB_OTG_FS2_FIFO;
        usbHostHandler.usbPower     = USB_OTG_FS2_PWR;
        ...
        NVIC_EnableIRQRequest(OTG_FS2_IRQn, 5, 0);
...
void USBH_HardwareReset(USBH_INFO_T* usbInfo)
{
    if (usbInfo->hostSpeed == USBH_SPEED_FS2)
    {
        RCM_EnableAHB1PeriphReset(RCM_AHB1_PERIPH_OTG_FS2);
        RCM_DisableAHB1PeriphReset(RCM_AHB1_PERIPH_OTG_FS2);
