...
int main(void)
{
    ...
    if (state == FAILED)
    {
        /* Turn off LED2 */
        BOARD_LEDOff(LED2);
        LOGI(tag, "> CAN1 polling test is FAILED \r\n");
    }
    else
    {
        /* Turn on LED2 */
        BOARD_LEDOn(LED2);
        LOGI(tag, "> CAN1 polling test is PASSED \r\n");
    }
    ...
    if (state == FAILED)
    {
        /* Turn off LED3 */
        BOARD_LEDOff(LED3);
        LOGI(tag, "> CAN1 interrupt test is FAILED \r\n");
    }
    else
    {
        /* Turn on LED3 */
        BOARD_LEDOn(LED3);
        LOGI(tag, "> CAN1 interrupt test is PASSED \r\n");
    }
...
uint8_t CAN_Polling(void)
{
    ...
    i = 0;
    while((CAN_PendingMessage(CAN1, CAN_RX_FIFO_0) < 1) && (i != 0xFFFF))
    {
        i++;
    }
...
uint8_t CAN_Interrupt_Init(void)
{
    ...
    CAN_DisableInterrupt(CAN1, CAN_INT_F0MP);
