...
USART_T* COM_USART[COMn] = {COM1_PORT, COM2_PORT, COM3_PORT};

GPIO_T* COM_TX_PORT[COMn] = {COM1_TX_GPIO_PORT, COM2_TX_GPIO_PORT, COM3_TX_GPIO_PORT};

GPIO_T* COM_RX_PORT[COMn] = {COM1_RX_GPIO_PORT, COM2_RX_GPIO_PORT, COM3_RX_GPIO_PORT};

const GPIO_PIN_SOURCE_T COM_TX_PIN_SOURCE[COMn] = {COM1_TX_PIN_SOURCE, COM2_TX_PIN_SOURCE, COM3_TX_PIN_SOURCE};

const GPIO_PIN_SOURCE_T COM_RX_PIN_SOURCE[COMn] = {COM1_RX_PIN_SOURCE, COM2_RX_PIN_SOURCE, COM3_RX_PIN_SOURCE};

const GPIO_AF_T COM_GPIO_AF[COMn] = {COM1_GPIO_AF, COM2_GPIO_AF, COM3_GPIO_AF};
 ...
 * @retval      None
 */
void BOARD_BuzzerInit(void)
{
    GPIO_Config_T GPIO_configStruct;

    RCM_EnableAHB1PeriphClock(BUZZER_GPIO_CLK);
    
    GPIO_configStruct.pin = BUZZER_PIN;
    GPIO_configStruct.mode = GPIO_MODE_OUT;
    GPIO_configStruct.speed = GPIO_SPEED_50MHz;
    GPIO_configStruct.otype = GPIO_OTYPE_PP;
    GPIO_configStruct.pupd = GPIO_PUPD_DOWN;
    GPIO_Config(BUZZER_GPIO_PORT, &GPIO_configStruct);
    
    GPIO_ResetBit(BUZZER_GPIO_PORT, BUZZER_PIN);
}
 ...
 * @retval      None
 */
void BOARD_BuzzerOn(void)
{
    GPIO_SetBit(BUZZER_GPIO_PORT, BUZZER_PIN);
}
 ...
 * @retval      None
 */
void BOARD_BuzzerToggle(void)
{
    BUZZER_GPIO_PORT->ODATA ^= BUZZER_PIN;
}
