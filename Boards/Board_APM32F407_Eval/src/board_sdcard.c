 ...
 *
 */
void BOARD_SDResetSDIOReg(void)
{
 ...
 *
 */
SD_STATUS_T BOARD_SDInit(SD_CARD_INFO_T *sdInfo)
{
    SD_STATUS_T status = SD_OK;
 ...
 *
 */
void BOARD_SDShowCardInfo(SD_CARD_INFO_T *sdInfo)
{
 ...
 *
 */
SD_STATUS_T BOARD_SDReadBlocks(SD_CARD_INFO_T *sdInfo, uint8_t *data, uint32_t blkAddr, uint32_t blkNum, uint32_t timeout)
{
    SD_STATUS_T status = SD_OK;
    SD_ERR_T errStatus = SD_ERR_NONE;
    ...
    if(sdInfo->state == SD_STATE_READY)
    {
                                     ...
                                     SDIO_FLAG_SBE)))
        {
            ...
            if(((BOARD_ReadTick() - tick) >= timeout) || (timeout == 0))
            {
                SDIO_ClearStatusFlag(SDIO_STATIC_FLAGS);
                sdInfo->state = SD_STATE_READY;
                return SD_TIMEOUT;
 ...
 *
 */
SD_STATUS_T BOARD_SDWriteBlocks(SD_CARD_INFO_T *sdInfo, uint8_t *data, uint32_t blkAddr, uint32_t blkNum, uint32_t timeout)
{
    SD_STATUS_T status = SD_OK;
    SD_ERR_T errStatus = SD_ERR_NONE;
    ...
    __IO uint32_t tick = BOARD_ReadTick();
 ...
 *
 */
uint32_t BOARD_SDReadCardState(SD_CARD_INFO_T *sdInfo)
{
    SD_ERR_T errStatus = SD_ERR_NONE;
 ...
 *
 */
uint32_t BOARD_SDReadState(SD_CARD_INFO_T *sdInfo)
{
    return sdInfo->state;
}
