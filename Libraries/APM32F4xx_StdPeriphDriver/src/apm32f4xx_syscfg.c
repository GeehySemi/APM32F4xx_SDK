...
void SYSCFG_ConfigEINTLine(SYSCFG_PORT_T port, SYSCFG_PIN_T pin)
{
    ...
    bitPos = 0x04 * (pin & 0x03);
    mask = (uint32_t)0x0F << bitPos;
    status = ((uint32_t)(port & 0x0F)) << bitPos;

    if ((pin >> 2) == 0)
    {
        SYSCFG->EINTCFG1 = (SYSCFG->EINTCFG1 & ~mask) | status;
    }
    else if  ((pin >> 2) == 1)
    {
        SYSCFG->EINTCFG2 = (SYSCFG->EINTCFG2 & ~mask) | status;
    }
    else if  ((pin >> 2) == 2)
    {
        SYSCFG->EINTCFG3 = (SYSCFG->EINTCFG3 & ~mask) | status;
    }
    else if  ((pin >> 2) == 3)
    {
        SYSCFG->EINTCFG4 = (SYSCFG->EINTCFG4 & ~mask) | status;
    }
}
