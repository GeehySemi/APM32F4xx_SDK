 ...
 * @attention
 *
 *  Copyright (C) 2021-2026 Geehy Semiconductor
 *
...
/*!
 * @brief  Wait until the write operation will be taken into account:
 *         at least four TX_CLK/RX_CLK clock cycles.
 *
 * @param  reg: pointer to the register to synchronize.
 *
 * @retval None
 */
static void ETH_RegWriteSync(__IO uint32_t *reg)
{
    __IO uint32_t temp;
...
uint32_t ETH_EnablePHYLoopBack(uint16_t addr)
{
    ...
    if(ETH_WritePHYRegister(addr,PHY_BCR,temp) == ETH_SUCCESS)
    {
        return ETH_SUCCESS;
    }
    else
...
void ETH_ResetWakeUpFrameFilterRegisterPointer(void)
{
    ...
    ETH_RegWriteSync(&ETH->PMTCTRLSTS);
}
